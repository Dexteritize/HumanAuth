...
def test_css_enhancements():
    ...
    scss_file = Path('/HumanAuth-FullStack/frontend/src/app/auth-page/auth-page.component.scss')
...
def test_build_compatibility():
    ...
    frontend_dir = Path('/HumanAuth-FullStack/frontend')
    ...
    print("ℹ️  To test the build, run: cd HumanAuth-FullStack/frontend && npm run build")
...
def main():
    ...
    if all(results):
        ...
        print("1. Start the backend: cd HumanAuth-FullStack && ./start.sh")
        print("2. Build frontend: cd HumanAuth-FullStack/frontend && npm run build")
