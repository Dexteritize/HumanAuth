...
# For Render deployment, add Render URLs to allowed origins if not already included
render_frontend_url = os.environ.get("RENDER_EXTERNAL_URL", "")
if render_frontend_url and render_frontend_url not in allowed_origins:
    allowed_origins.append(render_frontend_url)
    # Also add the URL without trailing slash if it has one
    if render_frontend_url.endswith("/"):
        allowed_origins.append(render_frontend_url[:-1])
